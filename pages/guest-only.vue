<template>
  <div class="w-full bg-light-cyan text-black p-3  justify-center my-10">
    ...
    <div class="w-full">
      <button class="bg-blue-500 mb-5 text-white p-2 rounded-md float-right" @click="handleOnClickOpenModal">Add
        Tenant</button>
    </div>
...
import addTenants from "@/pages/views/addTenants.vue"
...
const { data: tenants, refresh } = await useApiFetch<PaginatedResponse<Tenant>>('/api/v1/tenants')

const modal = useModal();
function handleOnClickOpenModal() {
  modal.open(addTenants, [
    {
      label: "Save",
      class: "px-8 py-2.5 leading-5 text-white transition-colors duration-300 transform bg-gray-700 rounded-md hover:bg-gray-600 focus:outline-none focus:bg-gray-600",
      callback: async (dataFromView) => {
