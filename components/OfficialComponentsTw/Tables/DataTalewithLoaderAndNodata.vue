                ...
                <table class="tw-min-w-full tw-leading-normal tw-text-gray-500 table-fixed">
                    ...
                    <tbody class="tw-bg-white dark:tw-bg-gray-800">
                        <tr v-if="props.loading"
                            class="tw-text-center tw-items-center tw-font-bold tw-p-8 odd:tw-bg-gray-100">
                            <td class="tw-p-4 tw-flex-1 tw-w-full tw-items-center tw-justify-between"
                                :colspan="`${displayedFieldKeys.length}`">
                                
                                <i class="fa-solid fa-spinner tw-relative tw-text-gray-700 tw-animate-spin tw-mr-1"></i>
                                LOADING
                            </td>
                        </tr>
                                ...
                                <template v-for="key in displayedFieldKeys">
                                    <Component :is="cellElement(key)"
                                        ...
                                        <slot :name="`cell(${key})`" :value="format(item, (key))" :item="item"
                                            :index="index" :format="(k) => format(item, k)">
                                            {{ format(item, (key)) }}
                                        </slot>
                                    </Component>
                                </template>
                        ...
                        <tr v-else class="tw-text-center tw-items-center tw-font-bold tw-p-8 odd:tw-bg-gray-100">
                            <td class="tw-p-4" :colspan="`${displayedFieldKeys.length}`">NO DATA</td>
                        </tr>
                    </tbody>
                    <!-- end:table body -->
                </table>
